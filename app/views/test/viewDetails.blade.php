        ...
        <div class="container-fluid">
            <div class="row less-gutter">
                <div class="col-md-11">
                    ...
                    @endif
                    @if ($test->isVerified() && (Auth::user()->can('edit_verified_results') ||Auth::user()->can('addend_result')) )
                ...
                <div class="col-md-6">
                    <div class="display-details">
                        <h3 class="view"><strong>{{ Lang::choice('messages.test-type',1) }}</strong>
                            {{ $test->test_type_id ? $test->testType->name : trans('messages.unknown') }}</h3>
                        <p class="view"><strong>{{trans('messages.visit-number')}}</strong>
                            {{$test->visit_id ? $test->visit->id : trans('messages.unknown') }}</p>
                        ...
                        <p class="view"><strong>{{trans('messages.lab-receipt-date')}}</strong>
                            {{$test->time_received?$test->time_received:'Not yet paid'}}</p>
                        ...
                        <p class="view-striped"><strong>{{trans('messages.request-origin')}}</strong>
                            ...
                            {{ trans("messages.in") }}
                            @else
                            {{ $test->visit->department .' '.$test->visit->visit_type.' '.$test->visit->ward}}
                            @endif</p>
                        ...
                        <p class="view"><strong>{{trans('messages.verified-by')}}</strong>
                            {{$test->verified_by ? $test->verifiedBy->name : trans('messages.verification-pending')}}</p>
                        @endif
                        ...
                        <div class="panel-body">
                            <div class="container-fluid">
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        {{$test->specimen->specimen_type ? $test->specimen->specimenType->name : trans('messages.pending') }}
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        {{$test->specimen->rejectionReason->reason? $test->specimen->rejectionReason->reason  : trans('messages.pending') }}
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-4">
