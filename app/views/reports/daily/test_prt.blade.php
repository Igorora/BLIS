		...
		<div id="test_records_div">
			...
			<table class="table table-bordered table-responsive">
				<tbody>
					...
					<tr>
						<!--<th>{{--{{ $test->visit->patient->id }}--}}</th>-->
						<td>{{ isset($test->visit->visit_number)?$test->visit->visit_number:$test->visit->id }}</td>
						<td>{{ $test->visit->patient->name }}</td>
						<!--<th>{{--{{ $test->getSpecimenId() }}--}}</th>-->
						<!--<th>{{--{{ $test->specimen->specimentype->name }}--}}</th>-->
						<!--<th>{{--{{ $test->specimen->time_accepted }}--}}</th>-->
						<td>{{ $test->testType->name }}</td>
						<td>
							@foreach($test->testResults as $result)
								<strong>{{Measure::find($result->measure_id)->name }}</strong>: {{$result->result.'; '}}
							@endforeach
							</br>
							{{ $test->interpretation }}
						</td>
						<td>{{ $test->tested_by? 'Tester: '.$test->testedBy->name.'; ' : 'Tester:pending' }}
							{{ $test->verified_by?'Verifier: '.$test->verifiedBy->name : 'Verifier:pending' }}

						</td>
						
						<!--<th>{{--{{ $test->interpretation }}--}}</th>-->
						<td>{{ $test->time_completed or trans('messages.pending') }}</td>
						<!--<th>{{--{{ $test->verified_by?$test->verifiedBy->name : trans('messages.verification-pending') }}--}}</th>-->
						<!--<th>{{--{{ $test->isCompleted() ||  $test->isVerified()?$test->getFormattedTurnaroundTime():trans('messages.pending') --}}</th>-->
					</tr>
					@empty
